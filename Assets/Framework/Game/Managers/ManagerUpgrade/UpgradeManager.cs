...
namespace U3dClient
{
    public class UpgradeManager : IGameManager
    {
        ...
        private readonly string m_BundleDotSuffixName = "." + CommonDefine.s_BundleSuffixName;
        ...
        private IEnumerator StartUpdateEnumerator(Action endAction, Action<long, long> progressAction)
        {
            ...
            using (var www =
                UnityWebRequest.Get(Path.Combine(FileUtils.s_WWWStreamingAssetsPath, versionFileName)))
            {
                yield return www.SendWebRequest();
                if (www.isNetworkError || www.isHttpError)
                {
                    Debug.Log("读取原始版本文件错误");
                }
                else
                {
                    ...
                    foreach (var data in datas)
                    {
                        ...
                        if (fileData.filePath != null) baseVersionDatas.Add(fileData.filePath, fileData);
                    }
                }
            }
                ...
                UnityWebRequest.Get(Path.Combine(m_ResUrl, versionFileName)))
            {
                ...
                else
                {
                    ...
                    foreach (var data in datas)
                    {
                        var fileData = new FileData();
                        GetFileData(data, ref fileData);
                        if (fileData.filePath != null) newVersionData.Add(fileData.filePath, fileData);
                    }
                }
            }
            ...
            foreach (var data in newVersionData)
            {
                ...
                var oldFileInfoPath = Path.Combine(FileUtils.s_PersistentDataPath,
                    newFileData.filePath.Replace(m_BundleDotSuffixName, "") + resInfoFileExten);
                var oldFileData = new FileData {filePath = null};
            ...
            foreach (var addFileData in addFileDatas)
            {
                ...
                var url = string.Format("{0}?{1}{2}", Path.Combine(m_ResUrl, filePath), fileMD5,
                    ...
                    UnityWebRequest.Get(url))
                {
                    ...
                    {
                        var fullFilePath = Path.Combine(FileUtils.s_PersistentDataPath, filePath);
